...
export default function Chatbot() {
  ...
  // Helper function to render message content (handles HTML for bot messages)
  ...
  return (
    <>
      {/* Google Fonts — add this once in your layout.tsx or _document.tsx instead */}
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@400;600&family=Jost:wght@300;400;500&display=swap');
      ...
      <button
        ...
        className="fixed bottom-7 right-7 w-16 h-16 rounded-full bg-yellow-700 border-none cursor-pointer flex items-center justify-center text-2xl z-[9999] transition-all duration-300 hover:scale-110 hover:-rotate-6 shadow-lg"
      >
        {open ? "✕" : "💬"}
        {/* {hasUnread && !open && (
          <span className="absolute top-1 right-1 w-3 h-3 bg-amber-500 rounded-full animate-badge" />
        )} */}
      </button>
      ...
      {open && (
        <div
          className="fixed bottom-28 right-7 w-[370px] max-h-[580px] flex flex-col rounded-2xl overflow-hidden z-[9998] animate-rise"
          style={{
            background: "#fdf8f2",
        ...
        >
          {/* Header */}
          <div
            className="px-5 py-[18px] relative overflow-hidden"
          ...
          >
            ...
            <div className="relative flex items-center gap-3">
              ...
              <div>
                <p className="font-cormorant text-xl font-semibold tracking-wide m-0 leading-none mb-1" style={{ color: "#f0dfc0" }}>
                  Fundi Juma
                </p>
                <span className="font-jost text-[11px] font-light uppercase tracking-widest flex items-center gap-1" style={{ color: "rgba(240,223,192,0.55)" }}>
                  <span
                    className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                    style={{ background: "#7ed4a4", boxShadow: "0 0 6px #7ed4a4" }}
                  />
          ...
          <div
            className="h-px"
            style={{ background: "linear-gradient(90deg, transparent, rgba(200,151,90,0.5), transparent)" }}
          />

          {/* Messages */}
          <div
            className="flex-1 overflow-y-auto px-3.5 pt-4 pb-2 flex flex-col gap-3 chat-scroll"
          ...
          >
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[80%] px-4 py-2.5 font-jost text-[13.5px] font-normal leading-relaxed break-words ${
                    msg.sender === "user"
                ...
                <span className="font-jost text-[10px] mt-1 px-1 tracking-wide" style={{ color: "rgba(100,60,20,0.35)" }}>
                  {msg.time}
                </span>
              </div>
            ))}
            ...
            {isTyping && (
              <div className="flex flex-col items-start animate-msg">
                <div
                  className="flex gap-1.5 items-center px-4 py-3 rounded-[18px] rounded-bl-[4px]"
                  style={{
                    background: "#fff",
                    border: "1px solid rgba(200,151,90,0.2)",
                  }}
                >
                  <span className="w-2 h-2 rounded-full animate-tdot1" style={{ background: "#c8975a" }} />
                  <span className="w-2 h-2 rounded-full animate-tdot2" style={{ background: "#c8975a" }} />
                  <span className="w-2 h-2 rounded-full animate-tdot3" style={{ background: "#c8975a" }} />
                </div>
              </div>
            )}
          ...
          {/* Quick Replies */}
          <div className="flex gap-1.5 flex-wrap px-3.5 py-2" style={{ background: "#fdf8f2" }}>
            {QUICK_REPLIES.map((qr) => (
              <button
                key={qr}
                onClick={() => sendMessage(qr)}
                className="font-jost text-[11.5px] font-medium tracking-wide px-3 py-1.5 rounded-full cursor-pointer transition-all duration-200 hover:scale-105 hover:bg-amber-50"
                style={{
          ...
          >
            <input
              className="flex-1 font-jost text-[13.5px] font-light rounded-xl px-3.5 py-2.5 outline-none transition-all duration-200 focus:ring-2"
              style={{
                color: "#3a2210",
                background: "#fdf8f2",
                border: "1px solid rgba(200,151,90,0.25)",
            ...
            <button
              ...
              className="w-11 h-11 rounded-xl border-none cursor-pointer flex items-center justify-center flex-shrink-0 transition-all duration-200 hover:scale-110 active:scale-95 disabled:opacity-35 disabled:cursor-not-allowed disabled:scale-100"
              style={{
                background: "linear-gradient(135deg, #c8975a, #a0733e)",
          ...
          {/* Footer */}
          <div
            className="text-center py-1.5 font-jost text-[9.5px] tracking-widest uppercase"
